...
const prense = { Away: "AWAY", present: "PRESENT" };
const notificationType = { init: "INIT", receivedGuesture: "RETRIEVED_GESTURE" };
...
module.exports = NodeHelper.create({
    ...
    socketNotificationReceived: function(notification, payload) {
        var self = this;
        if (notification === notificationType.init) {
            this._init(payload);
        }
    },
    // broadcast text messages to all subscribers (open web views)
    broadcast: function(str) {
        this.sendSocketNotification(notificationType.receivedGuesture, str);
    },
    ...
    saveEnergy: function(person) {
        ...
        if (person == prense.present && !self.hdmiOn) {
        ...
        else if (person == prense.Away && self.hdmiOn) {
    ...
    _init: function(payload) {
        ...
        this.WAIT_UNTIL_SLEEP = 1 * moduleConfig.monitorSleepTimeOut * 1000;
        ...
        var exec = require('child_process').exec;
        exec('ls /dev/' + moduleConfig.uartName + '*', function(error, stdout, stderr) {
            if (error !== null) {
                console.log(new Date() + ': exec error: ' + error);
            } else {
                ...
                const serialPort = new SerialPort(usbDev, {
                    baudRate: moduleConfig.uartSpeed
                });
